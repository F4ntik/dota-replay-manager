...
// Forward declare the struct types, then typedef pointers to them.
// This is a more C++ friendly way for opaque pointers when mixing C/C++.
struct SRepackMPQArchive;
typedef struct SRepackMPQArchive *MPQARCHIVE;
struct SRepackMPQFile;
typedef struct SRepackMPQFile *MPQFILE;
struct SRepackMPQLoader;
typedef struct SRepackMPQLoader *MPQLOADER;
struct SRepackMPQListFile;
typedef struct SRepackMPQListFile *MPQLISTFILE;
